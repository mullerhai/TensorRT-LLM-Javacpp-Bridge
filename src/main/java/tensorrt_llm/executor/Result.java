// Targeted by JavaCPP version 1.5.13: DO NOT EDIT THIS FILE

package tensorrt_llm.executor;

import java.nio.*;
import org.bytedeco.javacpp.*;
import org.bytedeco.javacpp.annotation.*;

import static tensorrt_llm.global.Executor.*;


/** \brief Struct that holds the generation result */
@Namespace("tensorrt_llm::executor") @Properties(inherit = tensorrt_llm.presets.ExecutorConfig.class)
public class Result extends Pointer {
    static { Loader.load(); }
    /** Default native constructor. */
    public Result() { super((Pointer)null); allocate(); }
    /** Native array allocator. Access with {@link Pointer#position(long)}. */
    public Result(long size) { super((Pointer)null); allocateArray(size); }
    /** Pointer cast constructor. Invokes {@link Pointer#Pointer(Pointer)}. */
    public Result(Pointer p) { super(p); }
    private native void allocate();
    private native void allocateArray(long size);
    @Override public Result position(long position) {
        return (Result)super.position(position);
    }
    @Override public Result getPointer(long i) {
        return new Result((Pointer)this).offsetAddress(i);
    }

    /** \brief Indicates if this is the final result for the request */
    public native boolean isFinal(); public native Result isFinal(boolean setter);

    /** \brief The output tokens for each beam */
    public native @ByRef Pointer outputTokenIds(); public native Result outputTokenIds(Pointer setter);

    /** \brief The cumulative log probabilities. Size beamSize. */
    public native @Optional Pointer cumLogProbs(); public native Result cumLogProbs(Pointer setter);

    /** \brief The log probabilities for each generated token. Size [beamSize, outputLen] */
    public native @Cast("std::vector<tensorrt_llm::executor::VecLogProbs>*") @Optional Pointer logProbs(); public native Result logProbs(Pointer setter);

    /** \brief The context logits. Size [promptLen, vocabSizePadded] */
    public native @Optional Tensor contextLogits(); public native Result contextLogits(Tensor setter);

    /** \brief The generation logits. Size [beamSize, maxTokens, vocabSizePadded] (non-streaming)
     *  or [maxTokens, beamSize, vocabSizePadded] (streaming and allGeneratedTokens)
     *  or [1, beamSize, vocabSizePadded] (streaming and non-allGeneratedTokens) */
    public native @Optional Tensor generationLogits(); public native Result generationLogits(Tensor setter);

    /** \brief Logits information for direct transfer when using fast logits */
    public native @Optional SpeculativeDecodingFastLogitsInfo specDecFastLogitsInfo(); public native Result specDecFastLogitsInfo(SpeculativeDecodingFastLogitsInfo setter);

    /** \brief The encoder output. Size [encoderLen, hiddenSize] */
    public native @Optional Tensor encoderOutput(); public native Result encoderOutput(Tensor setter);

    /** \brief The reason why the model stopped generating tokens for each beam in this request. Size [beamSize].
     *  Currently only supported when beamSize is 1 and when using BatchingType::kINFLIGHT. */
    public native @StdVector FinishReason finishReasons(); public native Result finishReasons(FinishReason setter);

    /** \brief The params of the context phase. */
    public native @Optional ContextPhaseParams contextPhaseParams(); public native Result contextPhaseParams(ContextPhaseParams setter);

    /** \brief The number of the decoding iterations used to generate the result.
     *  In autoregressive decoding, it is equal to the maximum length of the beam in outputTokenIds.
     *  In speculative decoding, might be less than maximum length of the beam in outputTokenIds as more than
     *  one token can be generated per iteration. Used for speculative decoding statistics. */
    public native int decodingIter(); public native Result decodingIter(int setter);

    /** \brief The average number of decoded tokens per iteration. For standard model it is 1.
     *  For speculative decoding model >= 1 -- number of draft tokens accepted per step + 1. */
    public native float avgDecodedTokensPerIter(); public native Result avgDecodedTokensPerIter(float setter);

    /** \brief The index of the output sequence of this result where 0 <= sequenceIndex < numReturnSequences.
     *  In beam search (beamWidth > 1), this index will be always zero because all beams to be returned are included
     *  in this result. */
    public native int sequenceIndex(); public native Result sequenceIndex(int setter);

    /** \brief Indicates if this is the final result for a given sequence in the request
     *  In beam search (beamWidth > 1), the value will always equal to the value of isFinal. */
    public native boolean isSequenceFinal(); public native Result isSequenceFinal(boolean setter);

    /** \brief Performance metrics if returnPerfMetrics is set in OutputConfig */
    public native @Optional RequestPerfMetrics requestPerfMetrics(); public native Result requestPerfMetrics(RequestPerfMetrics setter);

    /** \brief The additional outputs */
    public native @StdVector AdditionalOutput additionalOutputs(); public native Result additionalOutputs(AdditionalOutput setter);
}
